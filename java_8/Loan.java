public class Loan {
     ...
     */
    public Loan(double annualInterestRate, int numberOfYears, double loanAmount) { //проверка на положительные значения
        if (annualInterestRate <= 0) { throw new IllegalArgumentException("Годовая процентная ставка должна быть больше нуля."); }
        if (numberOfYears <= 0) { throw new IllegalArgumentException("Срок кредита должен быть больше нуля."); }
        if (loanAmount <= 0) { throw new IllegalArgumentException("Сумма кредита должна быть больше нуля."); }
